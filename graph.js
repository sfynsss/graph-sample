...
module.exports = {
  ...
  getFiles: async function(msalClient, userId) {
    const client = getAuthenticatedClient(msalClient, userId);
    try {
  ...
  upload: async function(msalClient, userId, files) {
    const client = getAuthenticatedClient(msalClient, userId);
    const filePath = path.join(__dirname, 'template', files+'.docx');
    // const { file } = files;
    // const fileName = file.name;

    try {
      // Upload the file to Microsoft Graph OneDrive
      const fileContent = fs.readFileSync(filePath);

      const fileName = path.basename(filePath);
      // const payload = {
      //   name: fileName,
      //   '@microsoft.graph.conflictBehavior': 'rename', // Specify conflict behavior if file already exists
      // };

      const response = await client
        .api('/me/drive/root:/' + fileName + ':/content')
        .put(fileContent, { headers: { 'Content-Type': 'application/octet-stream' } });
